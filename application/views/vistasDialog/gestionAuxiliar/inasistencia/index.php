<?php if (($bodyData->results)!=0) {
                            ...
                            <form name="registroMarca" id="registroMarca" method="POST" >                           
                                <table class="table table-striped table-bordered dt-responsive nowrap  " cellspacing="0" width="100%" id="dataTables-asistencia">
                                    <thead class="bg-success" >
                                     ...
                                     <th style="border: hidden;color: #3b752e;"><center>MENSAJE</center></th>
                                     <th style="border: hidden;color: #3b752e;"><center>DOCUMENTO ADJUNTO</center>  </th>
                                     <th style="border: hidden;color: #3b752e;"><center>RESPUESTA</center>  </th>
                                        </tr>
                                    </thead>                                   
                                    <tbody>       
                                    <?php $i=1;
    foreach ($bodyData->results as $resultado):?>     
                                        <tr>    
                                    <td style="width:2px;"><center><?=$i?></center></td>
                                    <td style="width:100px;"><?=$resultado['nombre']?></td>
                                    <td style="width:1px;"><center><strong><?=$resultado['grados'].'°'.$resultado['seccio']?></strong></center></td>
                                    <td style="width:5px;"><center><strong><?=$resultado['fecha']?></strong></center></td>
                            <td><center><textarea readonly class="form-control" style="width:500px;" rows="1"><?=$resultado['mensaj']?></textarea></center></td>
                                    <td style="width:1px;"><center>
                                    <a  class="ver"         data-codigo="<?=$resultado['id']?>" title="Ver archivo subido" href="javascript:"><span class="fa fa-search"></span> </a>
                                    </center></td>
                                    <td style="width:1px;">
                                    <center>
                                        ...
                                        <option <?=$selected; ?> value="<?=$result->id?>"><?=$result->nom_justificada?></option>
                                        <?php
                                        } ?>
                                        </select>
                                    </center>                               
...
$("#dataTables-asistencia").dataTable();	
        ...
        $('[id="respuesta[]"]').change(function(){
           ...
           var usu    = $(this).data("usu");
            ...
            $.post( "registrarRespuesta", { respuesta:respuesta,codigo:codigo,fecha:fecha,usu:usu } ); 
	...
	$(".btnMarca").click(function(){            
 		...
 		$("input[name='txtmarcado[]']:checked").each(function() {			
            ...
            $.ajax({
                ...
                $("#DIVVERDETALLE").dialog({ draggable: false });
                $("#DIVVERDETALLE").dialog({ resizable: false });                                           
