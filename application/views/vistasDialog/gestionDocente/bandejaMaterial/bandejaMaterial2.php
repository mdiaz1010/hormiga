                            ...
                            <div class="mask">
                              ...
                              <div class="tools tools-bottom">
                                  <a  href="<?= base_url(trim($result['ruta']))?>"  target="_blank" style=" outline: none;"   class="fa fa-link"></a>
                                        <a href="javascript:" data-id="<?=$result['id']?>" data-bimestre="<?=$bodyData->arrayBusqueda['id_bimestre']?>" data-curso="<?=$bodyData->arrayBusqueda['id_curso']?>" data-seccion="<?=$bodyData->arrayBusqueda['id_seccion']?>" data-grado="<?=$bodyData->arrayBusqueda['id_grado']?>"   class="editarMaterial"   class="img-responsive center-block"><i class="fa fa-pencil"></i></a>
                                        <a href="javascript:" data-id="<?=$result['id']?>" data-bimestre="<?=$bodyData->arrayBusqueda['id_bimestre']?>" data-curso="<?=$bodyData->arrayBusqueda['id_curso']?>" data-seccion="<?=$bodyData->arrayBusqueda['id_seccion']?>" data-grado="<?=$bodyData->arrayBusqueda['id_grado']?>"   class="eliminarMaterial" class="img-responsive center-block" data-ruta="<?=$result['ruta']?>"><i class="fa fa-times"></i></a>
                              </div>
                            </div>
                          </div>
                          <div class="caption">
...
<div id="DIVEDITARMATERIAL"   title="INTRANET EDUCATIVA :: ELIMINAR MATERIAL"></div>
 <div id="DIVcargando" title="EN PROCESO">
     <center><strong>Esperes estamos cargando la informacion...</strong>
...
$('#DIVcargando').dialog({
    ...
    $('#DIVcargando').dialog({ resizable: false });         
                    ...
                    buttons: {
                        "SI": function() {
                                ...
                                $.ajax({
                                    ...
                                    $("#bandejaMaterial2").load("<?= site_url('GestionDocente/verbandejaprof/') ?>",{bimestre:bimestre,curso:curso,seccion:seccion});
                                    }
                                    });
                ...
                $("#DIVEDITARMATERIAL").dialog({ draggable: false });
                $("#DIVEDITARMATERIAL").dialog({ resizable: false });  
                ...
                $("#DIVELIMINARMATERIAL").dialog({ draggable: false });
                $("#DIVELIMINARMATERIAL").dialog({ resizable: false });                       
	...
	$(".editarMaterial").click(function(){        
                ...
                $.ajax({
                    type: 'POST',
                    url: "editarMaterial",
                    data: {id:id,curso:curso,grado:grado,seccion:seccion,bimestre:bimestre},
	...
	$(".eliminarMaterial").click(function(){        
                ...
                $.ajax({
                    ...
                    data:{id:id,curso:curso,grado:grado,seccion:seccion,bimestre:bimestre,ruta:ruta},
                    success: function (datos) {
                        if(datos.length>0){
