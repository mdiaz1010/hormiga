...
class GestionAlumno extends CI_Controller {
    ...
    public function bandejaNota(){
        ...
        foreach($cursos as $cur){ $arrayCursos[]=$cur->id_curso;}
    ...
    public function consultarHorario(){
        ...
        foreach ($puesto_grado as $grados){// EXTRAIGO EL PUESTO Y LA NOTA DEL GRADO SECCION
            if($alu==$grados->id_alumno){$arraygrado=array('puesto'=>$sump,'nota'=>(int)$grados->nota);}$sump++;$p++;
        }        
        ...
        foreach ($puesto_salon as $salon){ // ESTRAIGO EL PUESTO Y LA NOTA DEL GRADO
            if($alu==$salon->id_alumno){$arraysalon=array('puesto'=>$sums,'nota'=>(int)$salon->nota);}$sums++;$s++;
        }        
        ...
        foreach ($puesto_colegio as $colegio){ // EXTRAIGO EL PUESTO Y LA NOTA DEL COLEGIO
            if($alu==$colegio->id_alumno){$arraycolegio=array('puesto'=>$sumc,'nota'=>round($colegio->nota,2));}$sumc++;$c++;
        }                
        $i=1;        
    ...
    public function editarAsistenciasAl(){
        ...
        foreach($_FILES['images']['error'] as $key => $error){
            if($error == UPLOAD_ERR_OK){
                $name = $_FILES['images']['name'][$key];
                $tipo = $_FILES['images']['type'][$key];
                $namegeneric = $i.$codigo."-".$nomarchi.$name;
                $i++;
                $searcharray = array(' ');
                $namegeneric = str_replace($searcharray, '', $namegeneric);
    ...
    public function bandejaNotaRecod(){
        ...
        $curso=$this->input->post("curso");
        $ano=$this->input->post("ano");       
        if($curso==false || $ano==false){echo "Ingrese el curso";return true;}
        ...
        foreach ($arraybimest as $conocer){
             if($conocer->nota>17){$resultado='Su nivel de rendimiento es considerado<strong> SATISFACTORIO</strong>';}
        else if($conocer->nota>13){$resultado='Su nivel de rendimiento es considerado en<strong> PROCESO</strong>';}
        else if($conocer->nota>10){$resultado='Su nivel de rendimiento es considerado en <strong>INICIO</strong>';}
        else                      {$resultado='Su nivel de rendimiento es considerado PREVIO <strong>INICIO</strong>';}
