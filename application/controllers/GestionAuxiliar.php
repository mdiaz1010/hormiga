...
public function editarInfo(){
    ...
    public function registrarObservacion(){
        ...
        if($tipo==1){$observacion="El alumno no ingresó al salon de clases";}else{$observacion="El alumno justifico su inasistencia";}
...
switch ($datos[0]->user){
    ...
    default: 
        ...
        foreach($cantidadCur as $canti){$sum++;}
        foreach ($trayectoSalon as $salon){$arraySalonNot[]=round(($salon->nota/($sum*$s)),2);} 
        foreach ($trayectoGrado as $grado){$arraySalonGra[]=round(($grado->nota/($sum*$p)),2);} 
        foreach ($trayectoColeg as $coleg){$arraySalonCol[]=round(($coleg->nota/($sum*$c)),2);} 
        foreach ($trayecto as $tray){$arrayNotA[]=array('nota'=>round($tray->nota/$sum),
                                                        'desc'=>$tray->desc);} 
